document.addEventListener("DOMContentLoaded", function () {
    ...
    const updateRowStatus = (newStatus, statusLabel) => {
        ...
        if (statusCell) {
            ...
            const color = colors[newStatus] || "bg-light text-dark";
            statusCell.innerHTML = `<span class="badge ${color}">${statusLabel}</span>`;
        }

        // Atualiza botão de excluir
    ...
    <div class="row align-items-center">
        ...
        <div class="col-md-7">
            ...
            <div class="text-muted">
                ${p.paid_at} • Registrado por: ${p.created_by}
            </div>
    ...
    document.addEventListener("click", function (e) {
        ...
        fetch(`/vendas/payments/${btn.dataset.paymentId}/delete/`, {
            ...
            .then(data => {
                ...
                else alert(data.error || "Erro ao remover pagamento.");
            })
    ...
    // ================= FORM DE PAGAMENTO =================
    // Mostra hint de transação conforme forma de pagamento
    ...
    // ================= SUBMIT =================
    document.getElementById("pay-submit-btn").addEventListener("click", function () {
        ...
        fetch(`/vendas/orders/${currentOrderPk}/payments/add/`, {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    // Desativa botão de excluir se status não for open/canceled
                    const deletableStatuses = ["open", "canceled"];
                    if (!deletableStatuses.includes(data.new_status)) {
                        const deleteBtn = document.querySelector(`#order-row-${currentOrderPk} .btn-outline-danger`);
                        if (deleteBtn) {
                            deleteBtn.disabled = true;
                            deleteBtn.classList.remove("btn-outline-danger");
                            deleteBtn.classList.add("btn-outline-secondary");
                            deleteBtn.title = "Pedido em produção — exclusão bloqueada";
                        }
                    }
                } else {
                    // Exibe erro de transação duplicada
    ...
    const resetForm = () => {
        ...
        document.getElementById("pay-transaction").value = "";
        document.getElementById("pay-notes").value       = "";
        document.getElementById("pay-transaction").classList.remove("is-invalid");
        document.getElementById("pay-transaction-hint").textContent = "(opcional)";
    };
