    ...
    if request.method == "POST":
        try:
            ...
            product = SinglePiece(
                ...
                # Elétrica
                has_electrical_component=request.POST.get("has_electrical_component")
                == "on",
...
def product_update(request, pk):
    ...
    if request.method == "POST":
        try:
            ...
            # Elétrica
            product.has_electrical_component = (
                request.POST.get("has_electrical_component") == "on"
            )
...
def product_details(request, pk):
    ...
    product = get_object_or_404(SinglePiece, pk=pk)
    return render(
        request, "catalogo/products/product_details.html", {"product": product}
    )
